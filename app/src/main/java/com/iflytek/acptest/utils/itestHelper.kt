...
class ItestHelper {
    ...
    fun runServer() {
        ...
        FileHandler.logger("main","iTest is prepared.")
    }
    ...
    fun startItest() {
         ...
         */
        Runtime.getRuntime().exec("am start -a android.intent.action.MAIN --user 0 -c android.intent.category.LAUNCHER -n iflytek.testTech.propertytool/.activity.BootActivity")
        println("Launch iTest.")
        FileHandler.logger("main","Launch iTest.")
    }

    //开启iTest监控
    fun monitorStart(vararg args: String, pkg: String, tag: String) {
         ...
         */
        val items = join(",", args)
        Runtime.getRuntime().exec("am broadcast -a monitorStart --user 0 --es monitor $items --es pkg $pkg --ei interval 1000")
        println("iTest has started monitoring: $items.")
        FileHandler.logger(tag,"iTest has started monitoring: $items.")
    }

    //结束监控
    fun monitorFinish(tag: String) {
        Runtime.getRuntime().exec("am broadcast -a monitorFinish --user 0")
        println("Finish the iTest monitor.")
        FileHandler.logger(tag,"Finish the iTest monitor.")
    }
